          ...
          >
            ...
            <div className="mt-auto pt-6 border-t border-slate-700">
              <Link
                to="/dashboard/profile"
                onClick={() => setMobileMenuOpen(false)}
                className="flex items-center gap-3 p-3 bg-slate-800/40 rounded-2xl group active:scale-95 transition-all"
              >
                <img
...
>>>>>>> cb901e2d6da2fc52e07232d798ead4058ae670ae
                </div>
              </Link>
            </div>
          </aside>
        ...
        {/* --- DESKTOP SIDEBAR --- */}
        <aside
          className={`${isSidebarOpen ? 'w-64' : 'w-30'} bg-[#1E293B] p-6 hidden md:flex flex-col border-r border-slate-700 shadow-2xl transition-all duration-300 h-screen sticky top-0`}
        >
          <div className="mb-10 flex items-center gap-3 overflow-hidden">
            <Link to="/" className="flex items-center gap-3 group">
              <img
          ...
          <div className="mt-auto pt-6 border-t border-slate-700">
            <Link
              to="/dashboard/profile"
              className="flex items-center gap-3 p-2 hover:bg-slate-800 transition-colors rounded-2xl overflow-hidden group"
            >
              <img
                referrerPolicy="no-referrer"
                src={userImg}
                alt="profile"
                className="w-10 h-10 rounded-full border-2 border-blue-500 object-cover shrink-0 group-hover:border-white transition-all"
              />
              {isSidebarOpen && (
                <div className="truncate">
                  <p className="text-sm font-bold truncate group-hover:text-white">
                    {userName}
                  </p>
                  <p className="text-[10px] text-blue-400 font-bold uppercase tracking-wider">
                    {userRole}
        ...
        {/* --- MAIN CONTENT AREA --- */}
        <div className="flex-1 flex flex-col min-h-screen bg-[#1E293B]/80">
          <header className="h-20 bg-[#1E293B]/80 backdrop-blur-md flex justify-between items-center px-4 md:px-8 border-b border-slate-700 sticky top-0 z-40">
            ...
            <button
              ...
              className="px-3 py-2 bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white rounded-xl transition-all font-bold text-xs md:text-sm flex items-center gap-2"
            >
              <LogOut size={16} />{' '}
