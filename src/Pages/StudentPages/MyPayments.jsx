...
const MyPayments = () => {
    ...
    const { data: myPayments = [], isLoading } = useQuery({
        ...
        queryFn: async () => {
            const res = await axiosSecure.get(`/payments/my-payments?class_name=${student?.class_name}`);
            return res.data;
        },
    ...
    if (isLoading) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
                ...
                <p className="text-slate-500 font-bold animate-pulse tracking-widest uppercase text-xs">Checking Records...</p>
            </div>
        );
    }
    ...
    const handlePayment = async (paymentInfo) => {
        ...
        axiosSecure.post('/payments/create-checkout-session', paymentInfo )
            .then(res => {
                console.log(res)
                if (res?.data?.url) {
    ...
    return (
        <div className="mt-8 md:mt-12 animate-in fade-in duration-700">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4 px-2">
                <div>
                    <h2 className="text-2xl md:text-3xl font-black text-white tracking-tight">Pending Dues</h2>
                ...
                <div className="flex items-center gap-3 bg-blue-600/10 border border-blue-500/20 px-4 py-2 rounded-2xl">
                    ...
                    <div>
                        <p className="text-[9px] text-slate-500 font-bold uppercase">Total Items</p>
                        <p className="text-sm font-black text-white">{myPayments.length} Records</p>
                    </div>
                </div>
            </div>
            ...
            {myPayments.length === 0 && (
                <div className="text-center py-24 bg-[#0F172A] rounded-[2.5rem] border border-slate-800 border-dashed">
                    ...
                    <p className="text-slate-400 font-bold">No pending payments found for your class.</p>
                </div>
            )}
                ...
                <table className="w-full text-left">
                    ...
                    <tbody className="divide-y divide-slate-800/50">
                        {myPayments.map((payment) => (
                            <tr key={payment._id} className="hover:bg-blue-600/5 transition-all group">
                                <td className="p-6">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <p className="text-sm font-bold text-white uppercase tracking-tight">{payment.payments_title}</p>
                                            <p className="text-[10px] text-slate-500 line-clamp-1 max-w-[250px]">{payment.description}</p>
                                        </div>
                                    </div>
                                </td>
                                <td className="p-6">
                                ...
                                <td className="p-6 text-center">
                                    <button onClick={()=> handlePayment(payment)} className="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-black rounded-xl transition-all shadow-lg shadow-blue-900/20 active:scale-95">
                                        PAY NOW
                                    </button>
                                </td>
                            </tr>
            ...
            <div className="md:hidden space-y-4 px-1">
                {myPayments.map((payment) => (
                    <div key={payment._id} className="bg-[#0F172A] border border-slate-800 rounded-3xl p-6 relative overflow-hidden group">
                        ...
                        <div className="relative z-10">
                            ...
                            <h3 className="text-lg font-black text-white leading-tight uppercase mb-1">{payment.payments_title}</h3>
                            <p className="text-xs text-slate-500 mb-5 line-clamp-2">{payment.description}</p>
                            
                            <div className="flex justify-between items-end border-t border-slate-800/50 pt-5">
                                <div>
                                    <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest mb-1">Total Amount</p>
                                    <p className="text-xl font-black text-white italic">৳{Number(payment.amount).toLocaleString()}</p>
                                </div>
                                <button className="p-3 bg-blue-600 text-white rounded-2xl shadow-xl shadow-blue-900/30">
