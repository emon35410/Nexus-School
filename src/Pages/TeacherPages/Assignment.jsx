    ...
    <aside className="bg-[#1E293B] border border-slate-800 p-8 rounded-[2.5rem] shadow-2xl  top-10">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div>
          ...
          ></textarea>
          {errors.instructions && <p className="text-rose-500 text-[10px] mt-1 ml-2 font-bold">{errors.instructions.message}</p>}
        </div>

        {/* Target Class Selection */}
        <div className="relative">
          <select 
            {...register("targetClass", { required: "Please select a class" })} 
          ...
          {errors.targetClass && <p className="text-rose-500 text-[10px] mt-1 ml-2 font-bold">{errors.targetClass.message}</p>}
        </div>
        ...
        <button type="submit" className="w-full py-3.5 bg-emerald-600 hover:bg-emerald-500 text-white font-black rounded-xl shadow-lg shadow-emerald-600/20 transition-all active:scale-95 uppercase tracking-widest text-xs">
          Assign to Students
        </button>
          </form>
    ...
    >
      ...
      <div className="flex justify-between items-start mb-4">
        ...
        <div className="text-right">
          <p className="text-[10px] text-slate-500 uppercase tracking-tighter leading-none">Marks</p>
          <p className="text-emerald-500 font-black text-xl">{a.totalMarks}</p>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4 pt-4 border-t border-slate-800/50">
        ...
        <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 text-slate-500">
                ...
                <div>
                    <p className="text-[9px] uppercase leading-none">Due Date</p>
                    <p className="text-xs font-semibold text-slate-300">{a.dueDate}</p>
                </div>
            </div>
            ...
            <button className="flex items-center gap-1.5 bg-slate-900 hover:bg-emerald-600 text-white py-1.5 px-3 rounded-lg transition-all border border-slate-700 hover:border-emerald-500 group/btn">
                <Eye size={14} />
                <span className="text-[10px] font-bold uppercase">Submissions</span>
            </button>
        </div>
